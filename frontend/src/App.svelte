<script>
  ...
  const unassignedCards = $derived(pinnedCards.filter((c) => !c.panelId));

  // Custom user-created panels
  const customPanels = $derived(dashboardStore.panels.filter((p) => p.id !== 'default'));
...
>
  ...
  <aside
    class="w-64 bg-[#090e17]/80 backdrop-blur-xl border-r border-cyan-500/20 flex flex-col justify-between p-4 z-30 shadow-[4px_0_24px_rgba(0,0,0,0.5)]"
  >
    <div class="space-y-6">
      <!-- App Brand Logo -->
      <div class="flex items-center gap-3 px-2 pt-2">
        <div
          class="relative flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-tr from-cyan-500 to-purple-600 shadow-[0_0_15px_rgba(6,182,212,0.5)]"
        >
          <Zap class="w-6 h-6 text-black fill-current animate-pulse" />
        </div>
        <div>
          <h1
            class="font-mono text-base font-bold tracking-wider bg-gradient-to-r from-cyan-400 via-purple-400 to-amber-400 bg-clip-text text-transparent"
          >
            LED SWARM
          </h1>
      ...
      <nav class="space-y-1">
        ...
        <button
          ...
          class="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl font-mono text-xs transition-all duration-200 cursor-pointer {activeTab ===
          'groups'
            ? 'bg-purple-500/10 text-purple-400 border border-purple-500/40 shadow-[0_0_15px_rgba(168,85,247,0.2)]'
            : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900/60'}"
        >
    ...
    <div class="glass-panel p-3.5 rounded-2xl space-y-2 border border-slate-800">
      ...
      <div class="w-full bg-slate-900 h-1.5 rounded-full overflow-hidden border border-slate-800">
        <div
          ...
          style="width: {store.devices?.length
            ? ((store.onlineDevices?.length || 0) / store.devices.length) * 100
            : 0}%"
        ></div>
      </div>
    </div>
  ...
  <main
    class="flex-1 flex flex-col min-w-0 overflow-hidden bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900/40 via-[#06090e] to-[#06090e]"
  >
    <!-- TOP CONTROL BAR -->
    <header
      class="h-16 border-b border-cyan-500/10 bg-[#090e17]/60 backdrop-blur-md px-6 flex items-center justify-between z-20"
    >
      ...
      <div class="flex items-center gap-6 glass-panel px-4 py-2 rounded-2xl border border-cyan-500/20">
        ...
        <button
          ...
          class="flex items-center gap-2 px-3 py-1 rounded-xl transition-all duration-200 font-mono text-xs cursor-pointer {masterPower
      ...
      {#if activeTab === 'dashboard'}
        ...
        <div class="space-y-6">
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs font-mono text-slate-400">
                Drag and drop cards into custom section panels or position them directly on the main canvas.
              </p>
            </div>
          ...
          <div
            ...
            class="space-y-3 rounded-3xl p-4 transition-all duration-300 border border-transparent {dragOverPanelId ===
            ''
            ...
            {#if pinnedCards.length === 0}
              <div class="glass-panel rounded-3xl p-12 text-center border border-dashed border-cyan-500/20">
                ...
                <p class="text-xs font-mono text-slate-500 mt-1">
                  Go to Settings -> Strips & Devices or Groups & Scenes to pin cards directly to your dashboard.
                </p>
              </div>
            {:else if unassignedCards.length === 0}
              <div class="glass-panel rounded-2xl p-6 text-center border border-dashed border-slate-800">
                <p class="text-xs font-mono text-slate-500">
                  No unassigned cards on main canvas. Drag cards here from any panel to move them back to the main
                  dashboard.
                </p>
              </div>
            {:else}
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
                {#each unassignedCards as item (item.id)}
                  <div
                    ...
                    class="{getCardSpanClass(
                      item.size
                    )} transition-all duration-300 h-full cursor-grab active:cursor-grabbing"
                  >
                    {#if item.type === 'device'}
                      <DeviceCard
                        device={item.data}
                      ...
                      <GroupCard
                        ...
                        onTogglePower={(id, pwr) =>
                          groupStore.setGroupState(
                            id,
                            pwr ? { on: true, bri: 200, mainseg: 0, seg: [{ id: 0, on: true }] } : { on: false }
                          )}
          ...
          {#each customPanels as panel (panel.id)}
            ...
            <div
              ...
              class="glass-panel rounded-3xl p-6 border transition-all duration-300 space-y-4 {dragOverPanelId ===
              panel.id
                ? 'border-purple-400/80 bg-purple-500/10 shadow-[0_0_30px_rgba(168,85,247,0.3)]'
                : 'border-slate-800/80'}"
            >
              <!-- Panel Header -->
        ...
        <div class="space-y-6">
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs font-mono text-slate-400">
                Combine multiple WLED strips into virtual synchronized groups and save multi-zone scene presets.
              </p>
            </div>
          ...
          <div class="space-y-3">
            ...
            {#if groupStore.groups.length === 0}
              <div class="glass-panel rounded-3xl p-12 text-center border border-dashed border-purple-500/20">
                ...
                <p class="text-xs font-mono text-slate-500 mt-1">
                  Click "Create Strip Group" to group WLED light strips together into synchronized zones.
                </p>
              </div>
            {:else}
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
