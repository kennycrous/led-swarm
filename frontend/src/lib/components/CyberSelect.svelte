<script>
  ...
  const selectedLabel = $derived(options[value] || (options.length === 0 ? 'Default' : options[0] || 'Select...'));
...
<div class="relative w-full {isOpen ? 'z-50' : 'z-10'}">
  ...
  >
    ...
    <ChevronDown
      class="w-3.5 h-3.5 text-slate-500 shrink-0 transition-transform {isOpen ? 'rotate-180 text-cyan-400' : ''}"
    />
  </button>
  ...
  {#if isOpen}
    ...
    <div
      class="absolute top-10 left-0 right-0 z-50 rounded-2xl border border-cyan-500/30 bg-[#090e17]/95 shadow-[0_0_25px_rgba(6,182,212,0.25)] backdrop-blur-xl overflow-hidden"
    >
        ...
        {:else}
          {#each options as opt, idx (idx)}
            <button
              ...
              class="w-full flex items-center justify-between px-2.5 py-1.5 rounded-xl text-[11px] text-left transition-all cursor-pointer {value ===
              idx
