<script>
  ...
  let { isOpen = false, allDevices = [], onClose = () => {}, onCreate = () => {} } = $props();
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
    <div class="glass-panel w-full max-w-md rounded-2xl p-6 border border-purple-500/40 shadow-2xl space-y-4">
      ...
      <form onsubmit={handleSubmit} class="space-y-4">
        ...
        <div>
          <label for="create-group-desc" class="block text-xs font-mono text-slate-400 mb-1"
          ...
          <div class="max-h-40 overflow-y-auto space-y-1.5 pr-1">
            {#each allDevices as dev (dev.id)}
              <button
                ...
                class="w-full flex items-center justify-between px-3 py-2 rounded-xl border text-xs transition-all text-left cursor-pointer {selectedDeviceIds.includes(
                  dev.id
                )
