...
<div class="space-y-6">
  <!-- Header Banner -->
  <div class="glass-panel rounded-3xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border border-cyan-500/20">
    <div>
      ...
      <p class="text-xs font-mono text-slate-400 mt-1">
        Complete network WLED strip inventory. Control dashboard visibility, rename strips, add direct IPs, or trigger mDNS discovery.
      </p>
    </div>
  ...
  <div class="glass-panel rounded-3xl p-6 border border-slate-800/80">
    {#if !devices || devices.length === 0}
      <div class="text-center py-12">
        ...
        <p class="text-xs text-slate-500 font-mono mt-1">Click "Discover Strips" or "Add Direct IP" to register strips on your network.</p>
      </div>
    {:else}
      <div class="space-y-3">
        {#each devices as dev (dev.id)}
          {@const isPinned = dashboardStore ? dashboardStore.isPinned(dev.id) : true}
          <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 rounded-2xl bg-slate-900/60 border border-slate-800 hover:border-cyan-500/30 transition-all duration-200">
            
            <!-- Left Info Section -->
            <div class="flex items-center gap-4">
              <!-- Online Health Pill -->
              <span class="w-3 h-3 rounded-full {dev.isOnline ? 'bg-emerald-400 shadow-[0_0_10px_rgba(52,211,153,0.5)] animate-pulse' : 'bg-rose-500'}" title={dev.isOnline ? 'Online' : 'Offline'}></span>
            ...
            <div class="flex items-center gap-3">
              ...
              {#if dashboardStore}
                <button 
                  ...
                  class="flex items-center gap-2 px-3 py-1.5 rounded-xl border text-xs font-mono transition-all cursor-pointer {isPinned ? 'bg-purple-500/20 text-purple-300 border-purple-500/40' : 'bg-slate-800/60 text-slate-500 border-slate-700 hover:text-slate-300'}"
                >
